...
# 注意：gene_router 同时挂载在这里（无 admin 校验）和下方 admin_router
# （有 require_org_role("admin") 校验）。genes.py 里的 admin 路由用字面量
# "/admin/..." 前缀，实际请求会命中这个无校验的挂载点。因此 genes.py 的
# admin 路由必须逐个自带 Depends(require_org_role("admin"))（见 6a314cf、
...
admin_router.include_router(spec_presets_router, prefix="/spec-presets",
    tags=["Admin - 规格预设"])
# 这个挂载点的 dependencies 校验对 genes.py 里的 admin 路由不生效（它们用
# 字面量 "/admin/..." 前缀，实际由上面 api_router 的无校验挂载点提供服务）。
# 真正生效的是 genes.py 每个 admin 路由自带的 Depends(require_org_role
# ("admin"))，这行 dependencies 只是保留给非 genes.py 的其他路由用。
