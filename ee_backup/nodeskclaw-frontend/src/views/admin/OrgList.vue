...
const filteredOrgs = computed(() => {
  if (!search.value) return adminStore.orgs
...
async function submitCreate() {
  ...
  try {
    await adminStore.addOrg(form.value)
...
async function submitDelete() {
  ...
  try {
    await adminStore.removeOrg(deleteConfirm.value)
          ...
          <tr v-for="org in filteredOrgs" :key="org.id" class="hover:bg-muted/20 transition-colors">
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                ...
                <span
                  v-if="!org.is_active"
                  class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-red-500/15 text-red-400"
                >
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-1">
                <button
                  class="p-1.5 rounded hover:bg-muted/60 text-muted-foreground hover:text-foreground transition-colors"
                  :title="t('admin.common.edit')"
                >
                  <Pencil class="w-3.5 h-3.5" />
                </button>
                <button
                  class="p-1.5 rounded hover:bg-red-500/10 text-muted-foreground hover:text-red-400 transition-colors"
      ...
      <div class="w-full max-w-md rounded-xl border border-border bg-card shadow-xl">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="col-span-2">
              ...
              <input
                ...
                :placeholder="t('admin.orgs.slugPlaceholder')"
              />
            </div>
            <div>
        ...
        <div class="flex items-center justify-end gap-2 px-5 py-4 border-t border-border">
          ...
          >
            ...
            <template v-else>
              <Check class="w-4 h-4 inline mr-1" />{{ t('admin.common.create') }}
            </template>
          </button>
        </div>
      </div>
    ...
    <div v-if="deleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div class="w-full max-w-sm rounded-xl border border-border bg-card shadow-xl">
        ...
        <div class="flex items-center justify-end gap-2 px-5 py-4 border-t border-border">
          ...
          <button
            class="h-9 px-4 rounded-lg bg-red-500 text-white text-sm font-medium hover:bg-red-600 transition-colors disabled:opacity-50"
